...
namespace eval ::23_patch_db {

    array set ::23_patch_db::options {
        --patch-db true
    }
    ...
    proc main { } {	
				...
				if {$read_data == "0100"} {
					...
					return -code error "  SORRY, NO DEX 4.25+ OFW ALLOWED !!!"
						...
						# set file [file join $path $self]
							# ::modify_coreos_file $file ::23_patch_db::Do_DEX_Patches
				} else {
					close $fp
				}
		}
		...
		set path $::CUSTOM_COSUNPKG_DIR
		set file [file join $path $self]
			::modify_coreos_file $file ::23_patch_db::Do_ISOLDR_Patches

		set self "spu_pkg_rvk_verifier.self"
		set path $::CUSTOM_COSUNPKG_DIR
		set file [file join $path $self]
			::modify_coreos_file $file ::23_patch_db::Do_PKG_Patches
    }

    # proc Do_DEX_Patches {self} {
        # log "Patching [file tail $self]"
			# ::modify_iso_file $self ::23_patch_db::DEX_elf_Patches
    # }
    ...
    proc Do_ISOLDR_Patches {self} {
        log "Patching [file tail $self]"
			::modify_iso_file $self ::23_patch_db::ISOLDR_elf_Patches
    }
    ...
    proc Do_PKG_Patches {self} {
        log "Patching [file tail $self]"
			::modify_iso_file $self ::23_patch_db::PKG_elf_Patches
    }
